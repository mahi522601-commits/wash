    ...
    <div className="p-4 rounded-2xl bg-white border border-brand-200 shadow-sm space-y-3 animate-fade-in text-xs">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-slate-100">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="text-xs font-bold text-[#1E1B4B]">Standard Garment Rate Card</h4>
      ...
      <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
        {filteredItems.map((item) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span className="font-mono font-bold text-xs text-[#6D28D9]">
                {formatCurrency(item.unitPrice)}
              </span>
              <button
                type="button"
                onClick={() => onStartBookingWithItem(item)}
                className="px-2 py-1 rounded-lg bg-[#6D28D9] hover:bg-[#5B21B6] text-white text-[10px] font-bold flex items-center gap-1 transition-colors"
              >
